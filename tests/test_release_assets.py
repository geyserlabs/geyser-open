...
def test_source_versions_and_tag_are_consistent() -> None:
    assert run("validate-source", "--tag", "v0.1.0").returncode == 0
    rejected = run("validate-source", "--tag", "v0.1.1")
    assert rejected.returncode != 0
    assert "must be 'v0.1.0'" in rejected.stderr
...
def test_manifest_checksum_verification_detects_tampering(tmp_path: Path) -> None:
    ...
    rejected = run("verify", "--asset-dir", str(tmp_path), "--tag", "v0.1.0")
    assert rejected.returncode != 0
